...
namespace strtk
{
                                      ...
                                      OutputIterator out)
   {
      ...
      while (0 < count)
      {
         ...
         int diff = static_cast<int>(pos - prev_pos); // modified
   ...
   private:
      ...
      struct row_processor
      {
         ...
         inline void operator()(const range_t& range)
         {
            ...
            row_end_index_ = static_cast<index_t>(row_start_index_ + token_count - 1); // modified
  ...
  template <typename OutputPredicate>
   ...
   namespace details
   {
      ...
      //strtk_register_sequence_iterator_type(std::vector) // modified
      ...
      inline bool string_to_type_converter_impl(Iterator& itr_external, const Iterator end, T& t, real_type_tag)
      {
         ...
         if (end != itr)
         {
            if ('.' == (*itr))
            {
               ...
               {
                  instate = true;
                  d += pow10(tmp_d,static_cast<int>(-std::distance(curr,itr))); // modified
               }
